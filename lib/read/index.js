...
var read = yuan.overload.Function(
	...
	function(OPT) {
		...
		do {
			...
			_print.text(OPT.prefix);

			// 读取用户输入。
			answer = OPT.prefix + _readline();
			_print.reset();
			if (!OPT.allowEmpty && answer == OPT.prefix) {
				if (yuan.object.has(OPT, 'default')) {
					answer = OPT.default;
					break;
				}
				else if (OPT.warnOnEmpty) {
					_print.warning('输入内容不能为空。');
				}
			}
